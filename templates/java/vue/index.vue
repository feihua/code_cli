<template>
  ...
  <div style=";background-color: white">
    ...
    <UpdateForm v-model="dialogUpdateFormVisible" @handleQuery="handleQuery"
                @handleEdit="dialogUpdateFormVisible = false" :record="recordVo"/>
  </div>
...
) =>
{
  ...
  let res: IResponse = await list
  {
    {
      class_name
    }
  }
  ({...data, ...searchParam.value, current: currentPage.value, pageSize: pageSize.value})
  tableData.value = {...res}
}

const handleQueryWithPageParam = async (data: {
{
  class_name
}
}

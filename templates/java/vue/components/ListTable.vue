<template>
  ...
  <el-table :data="props.tableData.data" table-layout="auto" @selection-change="handleSelectionChange" size="large">
    ...
    <el-table-column label="{{column.column_comment}}" prop="{{column.db_name}}"/>
    {% endfor %}
    <!--    {% raw %}-->
...
const handleEditView = (index: number, row: {
{
  class_name
}
}
...
) =>
{
  ElMessageBox.confirm(
    ...
    let res: IResponse = await remove
    {
      {
        class_name
      }
...
const handleSelectionChange = (recordVo: {
{
  class_name
}
}
